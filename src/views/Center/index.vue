                ...
                <tbody>
                  <tr v-for="(order,index) in cord.orderDetailList" :key="order.id">
                    ...
                    <template v-if="index === 0">
                      <td rowspan="0" width="8%" class="center">{{cord.consignee}}</td>
                      <td rowspan="0" width="13%" class="center">
                        <ul class="unstyled">
                          <li>总金额¥{{cord.totalAmount}}</li>
                          <li>在线支付</li>
                        </ul>
                      </td>
                      <td rowspan="0" width="8%" class="center">
...
import { reqGetOrderList } from "../../api/center";
export default {
  name: "Center",
  components: {
    Pagination,
  ...
  async mounted() {
    const res = await reqGetOrderList(this.page, this.limit);
    // console.log(res);
    this.records = res.records;
    this.total = res.total;
  },
  methods: {
    async handleSizeChange(page, limit) {
    ...
    async handleCurrentChange(page) {
      const res = await reqGetOrderList(page, this.limit);
      this.page = page;
      this.records = res.records;
      this.total = res.total;
    },
