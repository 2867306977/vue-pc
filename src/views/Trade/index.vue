<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="userAdd in userAddressList" :key="userAdd.id">
        <span
          :class="{
          username:true, 
          selected:userAdd.isDefault==='1'
          }"
        >{{userAdd.consignee}}</span>
        <p @click="toggleAdd(userAdd)">
    ...
    <div class="sub clearFix">
      <button class="subBtn" @click="submitOrders">提交订单</button>
    </div>
  </div>
</template>
...
export default {
  name: "Trade",
  methods: {
    ...mapActions("trade", ["getTrade"]),
    //切换地址信息
    toggleAdd(userAdd) {
      this.userAddressList.forEach((userAdd) => {
        userAdd.isDefault = "0";
      });
      userAdd.isDefault = "1";
    },
  ...
  computed: {
    ...
    //计算收货人信息
    userAddress() {
      const add = this.userAddressList.find((add) => {
