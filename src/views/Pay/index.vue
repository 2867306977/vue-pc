      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          <span class="fl">
            ...
            <em>{{$route.query.tradeNo}}</em>
          </span>
          <span class="fr">
            <em class="lead">应付金额：</em>
            <em class="orange money">￥{{$route.query.totalAmount}}</em>
          </span>
        </div>
      </div>
        ...
        <div class="otherpay">
          ...
          <Dialog title="立即支付" :visible.sync="visible" width="30%">
            ...
            <template #footer>
              ...
              <button @click="queryPayStatu">支付成功</button>
            </template>
          </Dialog>
        </div>
      </div>
...
import { reqGetOrderPayment, reqQueryPayStatus } from "../../api/order";
...
export default {
  ...
  methods: {
    //发送请求,二维码,支付成功则直接跳转页面
    async pay() {
      //发送请求,请求支付二维码
      this.visible = true;
      const res = await reqGetOrderPayment(this.$route.query.orderId);
      this.imgurl = await QRCode.toDataURL(res.codeUrl);
      //查询支付状态
      this.queryPayStatu();
    },
